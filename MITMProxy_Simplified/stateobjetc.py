...
class StateObject(serializable.Serializable):
    ...
    def get_state(self):
        ...
        for attr, cls in self._stateobject_attributes.items():
            ...
            if val is None:
                ...
                state[attr] = [x.get_state() for x in val] # Class is a List
                ...
                state[attr] = val # str, int , bool type
        return state
    ...
    def set_state(self, state):
        ...
        state = state.copy()
        for attr, cls in self._stateobject_attributes.items():
            val = state.pop(attr) #  返回attr对应的值然后删除这个值
            if val is None:
                ...
                if hasattr(self, "set_state"):
                    curr.set_state(val)
                elif hasattr(self, "from_state"):
                    obj = cls.from_state(val)
                    setattr(self, attr, obj)
                elif _is_list(cls):
                    cls = cls.__parameters[0] if cls.__parameters__ else cls.__args__[0]
                else:
                    # Other types such as int,str...
                    setattr(self, attr, cls(val))
        if state:
            raise RuntimeError("Unexpected State in __setstate)):{}".format(state))
